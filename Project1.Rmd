...
ageTeens$hour <- ifelse((minute(ageTeens$starttime) > 30), ageTeens$hour+0.5, ageTeens$hour)

numTeensHours <- seq(from = 0, to = 23.5, by = 0.5)
...
ggplot(data = teens, aes(x=numTeensHours, y=numTeensHoursAt)) + geom_line() + geom_smooth()
cat("This is a line graph showing how many teens (under 20) were riding citibikes at each half hour of the day.")
...
ageYoungAdults$hour <- ifelse((minute(ageYoungAdults$starttime) > 30), ageYoungAdults$hour+0.5, ageYoungAdults$hour)

numYoungAdultHours <- seq(from = 0, to = 23.5, by = 0.5)
numYoungAdultHoursAt <- numeric(length(numYoungAdultHours))
for(i in 1:length(numYoungAdultHours)) {
  numYoungAdultHoursAt[i] = length(which(ageYoungAdults$hour == numYoungAdultHours[i])) 
}

numYoungAdultHours$numHours <- as.data.frame(numYoungAdultHours)
numYoungAdultHoursAt <- as.data.frame(numYoungAdultHoursAt)
youngAdults <- cbind(numYoungAdultHours, numYoungAdultHoursAt)

ggplot(data = youngAdults, aes(x=numYoungAdultHours, y=numYoungAdultHoursAt)) + geom_line() + geom_smooth()
...
ageAdults$hour <- ifelse((minute(ageAdults$starttime) > 30), ageAdults$hour+0.5, ageAdults$hour)

numAdultHours <- seq(from = 0, to = 23.5, by = 0.5)
...
ggplot(data = adults, aes(x=numAdultHours, y=numAdultHoursAt)) + geom_line() + geom_smooth()
...
ageSeniors$hour <- ifelse((minute(ageSeniors$starttime) > 30), ageSeniors$hour+0.5, ageSeniors$hour)

numSeniorHours <- seq(from = 0, to = 23.5, by = 0.5)
...
ggplot(data = seniors, aes(x=numSeniorHours, y=numSeniorHoursAt)) + geom_line() + geom_smooth()
...
#tourists and age -- natives have subscription, tourists don't -- are more tourists of a particular age group? FIX THE BELOW
