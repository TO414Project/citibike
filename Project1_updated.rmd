...
ride$agegroup=cut(ride$age, 
                       ...
                       labels = c("Teens", "Young_Adults", "Adults", "Seniors"), 
                       right = FALSE)
...
model_Adults_snow <- lm(tripduration ~ poly(SNOW,1), data=combined_df[which(combined_df$agegroup==‘Adults’),])
newdat_Adults_snow = data.frame(SNOW = seq(min(combined_df[which(combined_df$agegroup==‘Adults’),]$SNOW), max(combined_df[which(combined_df$agegroup==‘Adults’),]$SNOW), length.out = 300))
newdat_Adults_snow$pred = predict(model_Adults_snow, newdata = newdat_Adults_snow)
...
ggplot()+geom_point(aes(x=SNOW,y=pred,color='Snow Impact on Teens'), newdat_Teens_snow)+geom_point(aes(x=SNOW,y=pred,color='Snow Impact on Young Adults’), newdat_Young_Adults_snow)+geom_point(aes(x=SNOW,y=pred,color='Snow Impact on Adults’), newdat_Adults_snow)+geom_point(aes(x=SNOW,y=pred,color='Snow Impact on Seniors'),newdat_Seniors_snow)
...
model_Young_Adults_PRCP <- lm(tripduration ~ poly(PRCP,1), data=combined_df[which(combined_df$agegroup==‘Young_Adults’),])
...
model_Adults_PRCP <- lm(tripduration ~ poly(PRCP,1), data=combined_df[which(combined_df$agegroup==‘Adults’),])
newdat_Adults_PRCP = data.frame(PRCP = seq(min(combined_df[which(combined_df$agegroup==‘Adults’),]$PRCP), max(combined_df[which(combined_df$agegroup==‘Adults’),]$PRCP), length.out = 300))
newdat_Adults_PRCP$pred = predict(model_Adults_PRCP, newdata = newdat_Adults_PRCP)
...
model_Adults_TMAX <- lm(tripduration ~ poly(TMAX,2), data=combined_df[which(combined_df$agegroup==‘Adults’),])
newdat_Adults_TMAX = data.frame(TMAX = seq(min(combined_df[which(combined_df$agegroup==‘Adults’),]$TMAX), max(combined_df[which(combined_df$agegroup==‘Adults’),]$TMAX), length.out = 300))
newdat_Adults_TMAX$pred = predict(model_Adults_TMAX, newdata = newdat_Adults_TMAX)
